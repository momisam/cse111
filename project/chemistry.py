# chemistry.py
# Enhancement Added:
# Program now also calculates and prints the TOTAL NUMBER OF PROTONS in the user’s molecule,
# using atomic numbers added to the periodic table dictionary.
...
def make_periodic_table():
    """Return a dictionary containing the periodic table.
       key = element symbol
       value = [name, atomic_mass, atomic_number]
    """
    periodic_table_dict = {
        "Ac": ["Actinium", 227, 89],
...
def main():
    ...
    symbol_qty_list = parse_formula(formula, periodic_table)

    molar_mass = compute_molar_mass(symbol_qty_list, periodic_table)
    print(f"{molar_mass:.5f} grams/mole")

    moles = sample_mass / molar_mass
    print(f"{moles:.5f} moles")

    # Added feature output
    total_protons = compute_total_protons(symbol_qty_list, periodic_table)
